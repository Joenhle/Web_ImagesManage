...
var Auth = {
	...
	register(e) {
		...
		e.preventDefault();
		//alert("检查 preventDefault() 是否被调用: " + e.isDefaultPrevented());
	},
	login(e) {
		flag=0;
	...
	forgot(e) {
		...
		Auth.vars.login_btn.innerText = '找回密码';       //重要按钮
        flag=1;
	...
	loginback(e) {
		...
		Auth.vars.login_btn.innerText = '登录';      //重要按钮
		Auth.vars.lowin_login.querySelector('form').setAttribute('action', "${pageContext.request.contextPath }/servlet/LoginServlet");
